/*----------------------------------------------------------------------
		...
		public class TrustsData : List<IMyThrust>
		{
			...
			public struct ThrustsValue
			{
				...
				public int Count;
				public void Add(IMyThrust val)
				{
					Count++;
					Max_pow += val.MaxThrust;
					EffectivePow += val.MaxEffectiveThrust;
					CurrentPow += val.CurrentThrust;
				}
		...
		void Main(string argument, UpdateType tpu)
		{
			...
			try
			{
				...
				if (Me.SurfaceCount > 0) TP = Me.GetSurface(0);
				...
				foreach (Base6Directions.Direction y in Enum.GetValues(typeof(Base6Directions.Direction)))
				{
					...
					if (shortV)
						...
						vals.EffectivePow / mas, vals.Max_pow, vals.Count, vals.EffectivePow));
					else
						s.AppendLine(string.Format(SYS, "Ускор: {0:0.0#}м/c. Cou: {2} = {1}/{4}\nCoof:{3:0.00}",
						vals.EffectivePow / mas, vals.Max_pow, vals.Count, vals.TecCoof, vals.EffectivePow));
				}

				var GrL = new List<IMyGyro>();
				new Selection("").FindBlocks<IMyGyro>(GridTerminalSystem, GrL);
				if (GrL.Count > 0)
				{
					float sum = GrL.Sum(x => x.CubeGrid.GridSizeEnum == MyCubeSize.Large ? 33600 : 448);
					s.AppendLine(string.Format(SYS, "\nГироскопы: {0} >> {1:0.00} / {2:0.00}", GrL.Count, sum, mas / sum));
					foreach (var x in GrL)
