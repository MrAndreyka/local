            ...
            switch (CP)
            {
                ...
                case 11:// Контроль
                    if (Rul.IsTarget())
                    {
                        ...
                        dist -= speed.GetDim(i) / 2 + nextspeed / 2; //дистанция до цели через секунду при торможении
                        ...
                        else if (speed.X >= maxSpeed * 0.999 || dist < pow*3 || CP == 11) // доп тяга не нужна
                        {
                            if (CP != 10)
                            {
        ...
        for (int i = 0; i < Len; i++)
        {
            ...
            switch (Arg)
            {
                ...
                case "dist":
                    {
                        Stoped();
                        IMyCameraBlock kam = null;
                        int dist = Right.IndexOf(':');
                        if (dist > -1)
                        {
                            var str = Right.Substring(0, dist);
                            kam = Sel(str).FindBlock<IMyCameraBlock>(GridTerminalSystem);
                            if (kam == null) ShowAndStop("Не найдена камера: " + str);
                            Right = Right.Substring(dist + 1);
                        }
                        else
                        {
                            kam = Sel("*").FindBlock<IMyCameraBlock>(GridTerminalSystem, x => x.IsActive);
                            if (kam == null) ShowAndStop("Не установлена текущая камера");
                        }
                        if (kam == null) return;
                        dist = 20000;
                        if (!string.IsNullOrWhiteSpace(Right))
...
void Note(string txt, bool unical = true)
{
    var str = RemCon.CustomData;
    if (unical && str.IndexOf(txt) >= 0) return;
